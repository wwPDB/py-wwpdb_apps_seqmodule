    ...
    for alignIndex in alignIndicesList:
        ...
        for i in range(0, len(alignIndex)):
            ...
            if intIdx >= len(seqList[i]):
                return "getSeqAlignment: Align index out of range for '" + labelIndexDict[i] + "': index=" + str(intIdx) + \
                       ", sequence length=" + str(len(seqList[i])) + ".",[]
            #
        #
    #
    alignList = []
...
def printAlignment(lfh=sys.stderr, alignList=[], shortFlag=True):
    ...
    for alignTup in alignList:
        ...
        for sTup in alignTup:
            ...
            #
            if shortFlag:
                lfh.write("(%s %5s %6s %6s %6s %4s %s)" % ("'" + sTup[0] + "'", "'" + sTup[1] + "'", "'" + sTup[2] + "'", \
                          "'" + str(sTup[3]) + "'", "'" + str(sTup[4]) + "'", "'" + str(sTup[6]) + "'", "'" + sTup[7] + "'"))
            else:
                lfh.write("(%s %5s %6s %6s %6s %35s %4s %s)" % ("'" + sTup[0] + "'", "'" + sTup[1] + "'", "'" + sTup[2] + "'", \
                          "'" + str(sTup[3]) + "'", "'" + str(sTup[4]) + "'", "'" + str(sTup[5]) + "'", "'" + str(sTup[6]) + "'", \
                          "'" + sTup[7] + "'"))
            #
            first = False
        #
