...
class AlignmentStatistics(object):
    ...
    def __updateAuthXyzAlignments(self):
        ...
        for gId in groupIdList:
            ...
            #
            if (self.__verbose):
                self.__lfh.write("+AlignmentStatistics.__updateAuthXyzAlignments() auth sequence entity group %s instance %s version %s length %d\n" 
            ...
            for seqId in seqIdList:
                ...
                if (self.__verbose):
                    self.__lfh.write("+AlignmentStatistics.__updateAuthXyzAlignments() xyz  sequence entity group %s instance %s version %s length %d\n" 
            ...
            #
            for seqId in seqIdList:
                if (not seqId in seqIdExistList):
                    continue
        ...
        if (self.__debug):
            for kTup,v in resultD.items():
                self.__lfh.write("  -  xyz seq  partId %d alt %d ver %d seqId %s seq length %d align length %d matches %d matches w/ gaps %d sBeg %s sEnd %s\n"
        ...
        for gId in groupIdList:
            ...
            for partId in partIdList:
                ...
                if (self.__debug):
                    for kTup,v in resultD.items():
                        if kTup[2] == partId:
                            self.__lfh.write("  -  ref seq  partId %2d alt %3d ver %d chainId %s length %5d align length %5d matches %5d matches w/ gaps %5d\n"
