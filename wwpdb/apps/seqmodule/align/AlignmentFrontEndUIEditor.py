...
class AlignmentFrontEndUIEditor(object):
    ...
    def __editMove(self):
        ...
        try:
            ...
            if (self.__verbose):
                traceback.print_exc(file=self.__lfh)
                self.__lfh.write("+AlignmentFrontEndUIEditor.__editMove()  Missing source %s or destination  %s  for session id %s\n"
        ...
        #
        if (self.__verbose):
            self.__lfh.write("+AlignmentFrontEndUIEditor.__editMove()  Source residue Id %s destination residue Id  %s  for session id %s\n"
                             % (srcResidueId,dstResidueId,self.__sessionId))
        # 
    ...
    def __editAny(self):
        ...
        try:
            ...
            if (self.__verbose):
                traceback.print_exc(file=self.__lfh)
                self.__lfh.write("+AlignmentFrontEndUIEditor.__editAny()  Missing input residueId %s value %s prior value %s session id %s\n"
    ...
    def __editDetails(self):
        ...
        try:
            ...
            if (self.__verbose):
                traceback.print_exc(file=self.__lfh)
                self.__lfh.write("+Alignment.__editDetails()  Missing input residueId %s value %s prior value %s session id %s\n"
                                 % (residueId,editValue,priorValue,self.__sessionId))
            #
            rD=self.__makeResponseDict(id=residueId,val=editValue,val3=editValue,editType='error',editOpId=None,newId=None)
            return rD
