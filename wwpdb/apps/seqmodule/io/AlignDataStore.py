...
import sys, cPickle, time, os, os.path, traceback
...
class AlignDataStore(object):
    ...
    def serialize(self):
        try:
            ...
            for ii in range(len(self.__seqAlignList)):
                ...
                oList.append([self.__seqAlignList[ii][0],t,self.__seqAlignList[ii][2],self.__seqAlignList[ii][3],self.__seqAlignList[ii][4] ])
            cPickle.dump(oList,fb,self.__pickleProtocol)
            fb.close()
        except:
            pass
    ...
    def deserialize(self):
        try:
            ...
            self.__seqAlignList=cPickle.load(fb)
            for ii in range(len(self.__seqAlignList)):
                sLab=SequenceLabel()
