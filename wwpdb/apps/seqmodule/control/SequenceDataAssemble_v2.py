...
#
import json, multiprocessing, os, shutil, string, sys, time, traceback
...
class SequenceDataAssemble(UpdateSequenceDataStoreUtils):
    ...
    def doAssemble(self, calPolyLink=True, doRefSearch=False, doAutoProcess=False):
        ...
        startTime = time.time()
        if self._verbose:
            self._lfh.write("+SequenceDataAssemble loadSequenceDataStore for sessionId %s started at %s \n" % \
        ...
        for newId,newD in currEntityD.items():
            for oldId,oldD in prevEntityD.items():
                ...
                if ("seqdb-match" in prevSavedFilePathMap) and ( oldId in prevSavedFilePathMap["seqdb-match"]):
                    ...
                    importFilePath = self.__pI.getFilePath(dataSetId=self.__dataSetId, contentType=tupL[0], formatType=tupL[1], \
                                                           fileSource="session", partNumber=newId)
    ...
    def __createSeqMatchFileAndMisMatchPickleFile(self, sortedEntityIdList, entityD, ownRefD, eSSRefD, prevEntityIdList, prevMisMatchList, prevNotFoundMatchList):
        ...
        try:
            ...
            #
            dataBlock.append(self.__writeGeneralCifCategory("pdbx_sequence_annotation", ( "entity_id", "entity_matched", "matched_entry_id", "matched_pdb_id", \
                             "matched_entity_id", "reference_db_name", "reference_db_code", "coord_seq_mismatch", "coord_seq_mismatch_message" ), annList))
            #
            blockList = []
    ...
    def __createDefaultAlignmentObject(self, dataD):
        ...
        """
        alignUtil = AlignmentTools(reqObj=self._reqObj, entityId=dataD["id"], pathInfo=self.__pI, seqDataStore=self.getSequenceDataStoreObj(), \
                                   deserializeFlag=False, verbose=self._verbose, log=self._lfh)
    ...
    def __updateSelections(self, selectedEntityIdList):
        ...
        removeList = []
        for entityId in selectedEntityIdList:
            if (not entityId in self._entityAlignInfoMap) or ("alignids" not in self._entityAlignInfoMap[entityId]):
                continue
            #
