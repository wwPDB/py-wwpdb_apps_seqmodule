    ...
    def copyFiles(self, inputFileSource="archive", inputWfInstanceId=None, outputFileSource="session", outputWfInstanceId=None, versionIndex=3, \
        ...
        for fileType in fileTypeList:
            ...
            if not os.access(inputFilePath, os.R_OK):
                ...
                continue
            #
            outputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=outputWfInstanceId, contentType=fileType[0], formatType=fileType[1], \
                                                  fileSource=outputFileSource, versionId=fileType[versionIndex])
        ...
        if includePolyLinkFile:
            inputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=inputWfInstanceId, contentType=self.__polyLinkType[0], \
                                                  formatType=self.__polyLinkType[1], fileSource=inputFileSource)
        ...
        if entityIdList:
            for eId in entityIdList:
                for fileType in self.__seqFileTypeList:
                    inputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=inputWfInstanceId, contentType=fileType[0], formatType=fileType[1], \
                                                          fileSource=inputFileSource, partNumber=eId)
                    if not os.access(inputFilePath, os.R_OK):
                        continue
                    #
                    outputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=outputWfInstanceId, contentType=fileType[0], formatType=fileType[1], \
                                                           fileSource=outputFileSource, versionId=fileType[versionIndex], partNumber=eId)
                    afpL.append( ( inputFilePath, outputFilePath, fileType[2] ) )
                #
            #
        else:
            gId = 1
            while True:
                inputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=inputWfInstanceId, contentType=self.__seqFileTypeList[1][0], \
                                                      formatType=self.__seqFileTypeList[1][1], fileSource=inputFileSource, partNumber=str(gId))
                if os.access(inputFilePath, os.R_OK):
                    outputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=outputWfInstanceId, contentType=self.__seqFileTypeList[1][0], \
                                                           formatType=self.__seqFileTypeList[1][1], fileSource=outputFileSource, \
                                                           versionId=self.__seqFileTypeList[1][versionIndex], partNumber=str(gId))
                    afpL.append( ( inputFilePath, outputFilePath, self.__seqFileTypeList[1][2] ) )
                #
                inputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=inputWfInstanceId, contentType=self.__seqFileTypeList[0][0], \
                                                      formatType=self.__seqFileTypeList[0][1], fileSource=inputFileSource, partNumber=str(gId))
                if os.access(inputFilePath, os.R_OK):
                    outputFilePath = self.__pI.getFilePath(self.__identifier, wfInstanceId=outputWfInstanceId, contentType=self.__seqFileTypeList[0][0], \
                                                           formatType=self.__seqFileTypeList[0][1], fileSource=outputFileSource, \
                                                           versionId=self.__seqFileTypeList[0][versionIndex], partNumber=str(gId))
