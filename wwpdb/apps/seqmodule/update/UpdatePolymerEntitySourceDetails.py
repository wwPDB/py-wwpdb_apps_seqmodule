...
class UpdatePolymerEntitySourceDetails(object):
    ...
    def __init__(self, reqObj=None, verbose=False, log=sys.stderr):
        ...
        self.__entityReviewItemList = ["ENTITY_DESCRIPTION", "ENTITY_SYNONYMS", "SOURCE_ORGANISM", "SOURCE_COMMON_NAME",
                                       "SOURCE_TAXID", "SOURCE_GENE_NAME", "SOURCE_STRAIN", "SOURCE_VARIANT", "ENTITY_ENZYME_CLASS",
                                       "ENTITY_FRAGMENT_DETAILS", "ENTITY_MUTATION_DETAILS", "ENTITY_DETAILS",
                                       "HOST_ORG_SOURCE", "HOST_ORG_COMMON_NAME", "HOST_ORG_STRAIN",
                                       "HOST_ORG_TAXID", "HOST_ORG_VECTOR", "HOST_ORG_VECTOR_TYPE", "HOST_ORG_PLASMID",
                                       "HOST_ORG_CELL_LINE", "HOST_ORG_VARIANT", "ANNO_EDIT_DB_NAME", "ANNO_EDIT_DB_CODE",
                                       "ANNO_EDIT_DB_ACCESSION", "ANNO_EDIT_DB_ALIGN_BEGIN", "ANNO_EDIT_DB_ALIGN_END",
                                       "SOURCE_METHOD", "CURRENT_AUTH_SELECT_ID", "CURRENT_REF_SELECT_ID"]
    ...
    def updateAuthEntityDetails(self, selectIdList=None):
        ...
        for entityId in entityIdList:
            ...
            for partId in partIdList:
                ...
                self.__lfh.write("+UpdatePolymerEntitySourceDetails.updateAuthEntityDetails() entityId %r partId %r refSelectId %r curRefId %r hasEdit %r\n" \
                              % (entityId, partId, refSelectId, curRefId, hasEdit))
                #
                if curRefId == refSelectId and hasEdit:
                    self.__lfh.write("+UpdatePolymerEntitySourceDetails.updateAuthEntityDetails() skip update entityId %r partId %r curRefId %r hasEdit %r\n" \
...
%(db_align_range)s
        ...
        if norineFlag:
            ...
            if authFD.has_key('ANNO_EDIT_DB_NAME') and authFD['ANNO_EDIT_DB_NAME'].upper() == 'NOR':
                db_name_class = ""
            ...
            #
            pD['db_name'] = '<span id="ANNO_EDIT_DB_NAME" class="ief my-editable-cell ' + db_name_class + '" data-ief-edittype="select"  data-ief-selectvalues=\'' + db_name_value_select \
                          + '\'>' + db_name_value_curr + '</span>'
            #
            ...
            for items in ( ( 'db_code', 'ANNO_EDIT_DB_CODE' ), ( 'db_accession', 'ANNO_EDIT_DB_ACCESSION' ) ):
                ...
                if authFD.has_key(items[1]) and authFD[items[1]]:
                    styleClass = ""
            ...
            for sId in self.__selectIdList:
                if sId.startswith('auth'):
                    ...
                    if instanceId == seqInstId and seqPartId == 1:
                        ...
                        self.__lfh.write("+UpdatePolymerEntitySourceDetails._getCurrentAuthSelection() returns: entity %r instance %r partId %r altId %r version %r\n" %
    ...
    def __getCurrentRefSelection(self, entityId, partId=1):
        try:
            ...
            for sId in self.__selectIdList:
                if sId.startswith('ref'):
                    ...
                    if instanceId == seqInstId and partId == seqPartId:
                        ...
                        self.__lfh.write("+UpdatePolymerEntitySourceDetails._getCurrentRefSelection() returns: entity %r instance %r partId %r altId %r version %r\n" %
                                         (entityId, seqInstId, seqPartId, seqAltId, seqVersion))
            ...
            self.__lfh.write("+UpdatePolymerEntitySourceDetails._getCurrentRefSelection() failed for selectList %r entityId %s partId %r\n" %
                (self.__selectIdList, entityId, partId))
            traceback.print_exc(file=self.__lfh)
        #
        self.__lfh.write("+UpdatePolymerEntitySourceDetails._getCurrentRefSelection() no return for entityId %r instanceId %r partId %r selectIdList %r\n" %
