...
class UpdatePolymerEntityPartitions(object):
    ...
    def makePolymerEntityPartEditForm(self, entityId, entryId=''):
        ...
        <div class="width50 fltrgt"> Add instances from entity:&nbsp;&nbsp; <span id="merge_instance_1"  class="ief greyedout" data-ief-edittype="select" data-ief-selectvalues='%s'>%s</span></div>
        <br />
        <br />
    ...
    def polymerEntityPartEditFormResponder(self):
        ...
        try:
            ...
            for partId in range(1, numParts + 8):
                ...
                if ((not seqPartType) or (seqPartType == self.__placeHolderValue) or (not seqBegin) or (seqBegin == self.__placeHolderValue) or \
                    (not seqEnd) or (seqEnd == self.__placeHolderValue)):
                    continue
                #
                if partId > numParts:
            ...
            for partId in range(1, numParts + 8):
                ...
                if pOrgD.has_key(partId) and pD.has_key(partId) and pOrgD[partId] != pD[partId]:
                ...
                elif (pD.has_key(partId) and (not pOrgD.has_key(partId))) or ((not pD.has_key(partId)) and pOrgD.has_key(partId)):
                    updatePartitionFlag = True
                    break
                #
            #
            ...
            for sId in self.__selectIdList:
                if sId.startswith('auth'):
                    ...
                    if instanceId == seqInstId and seqPartId == 1:
                        ...
                        self.__lfh.write("+UpdatePolymerEntityPartitions._getCurrentAuthSelection() returns: entity %r instance %r partId %r altId %r version %r\n" %
    ...
    def __updatePolymerEntityPartitions(self, entityId, partD, seq1=None, updateSequenceFlag=False):
        ...
        if (self.__verbose):
            self.__lfh.write("\n\n+UpdatePolymerEntityPartitions.__updatePolymerEntityPartitions() Starting for entity %s sequence update flag %r part dictionary\n%r\n" %
                             (entityId, updateSequenceFlag, partD,))
        #
        seqBegMin = 100
        seqEndMax = 0
        nP = 0
